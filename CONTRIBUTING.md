...
CHINJU は、安全仕様と実装変更を同期させるため、現時点では monorepo を採用しています。

以下のいずれかを満たす場合にのみ、分割を検討します。
- チームが完全に分かれ、独立したリリース周期が必要になったとき
- 権限分離をより強く求めるとき（例: enclave 系コードの別管理）
- CI時間やレビュー競合が運用上の限界に達したとき
